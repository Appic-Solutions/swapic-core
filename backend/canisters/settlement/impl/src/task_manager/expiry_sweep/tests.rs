...
use settlement_api::types::quote::{quote_bytes, GasMode, Quote};
...
fn the_sweep_drops_a_quote_once_its_permit_window_has_closed() {
    ...
    let hash =
        pending_quotes::register(q.clone(), q.expires_at_s - 5).expect("a live quote registers");
...
fn the_sweep_keys_eviction_on_the_expiry_and_not_on_the_registration() {
    ...
    let hash = pending_quotes::register(q.clone(), opened_at).expect("a live quote registers");
...
fn a_halted_sweep_still_drops_stale_quotes() {
    ...
    let hash = pending_quotes::register(q.clone(), q.expires_at_s).expect("a live quote registers");
